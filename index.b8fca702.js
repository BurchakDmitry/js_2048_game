    ...
    if (e.target.textContent === "Start") {
        ...
        document.addEventListener("touchend", (ev)=>{
            ...
            if (Math.abs(diffX) > Math.abs(diffY)) {
                if (diffX > threshold) game.moveRight();
                else if (diffX < -threshold) game.moveLeft();
            } else {
                if (diffY > threshold) game.moveDown();
                else if (diffY < -threshold) game.moveUp();
            }
            tableFormatting();
        }, false);
        return;
...
function tableFormatting() {
    tableCell.forEach((cell, index)=>{
        ...
        else loseMessage.classList.toggle("hidden");
